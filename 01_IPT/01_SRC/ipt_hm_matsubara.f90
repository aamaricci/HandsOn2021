program hmipt_matsubara
  ...
  complex(8)                          :: zeta
  !
  complex(8),dimension(:),allocatable :: fg0,sigma,fg
  real(8),dimension(:),allocatable    :: fg0_tau,sigma_tau,fg_tau
  real(8),dimension(:),allocatable    :: wm
  real(8)                             :: dens,docc,z
  complex(8),dimension(:),allocatable :: fg0_prev
  !
  ...
  !
  allocate(fg0_tau(0:L))  ;fg0_tau=0d0
  allocate(sigma_tau(0:L));sigma_tau=0d0
  !
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     !Set ReSigma==0 (particle-hole symmetry improve stability ;-)
